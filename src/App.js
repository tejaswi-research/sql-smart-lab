...
function App() {
  ...
  {/* --- 1. SMART SUGGESTIONS (The New Curiosity Layer) --- */}
    ...
    <div style={{ marginTop: '20px', padding: '15px', background: 'rgba(79, 172, 254, 0.1)', borderLeft: `4px solid ${colors.mellowBlue}`, borderRadius: '4px' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
        {[
          ...
          <button key={i} onClick={() => { setQuery(opt.sql); runQuery(opt.sql); }} style={{ padding: '6px 12px', fontSize: '11px', background: '#222', border: `1px solid ${colors.border}`, color: '#fff', borderRadius: '20px', cursor: 'pointer' }}>
            {opt.label}
          </button>
        ))}
      </div>
  ...
  {/* --- 2. STRUCTURAL PREVIEW (Your Original Active Schema Functionality) --- */}
  {activeSchema && (
  ...
  {/* --- 3. THE BEFORE/AFTER COMPARISON (Your Original Results Logic) --- */}
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ background: colors.sidebar, padding: '15px', borderRadius: '8px', border: `1px solid ${colors.border}`, overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
            ...
            <tbody>{sources[0].data.length > 0 ? sources[0].data.map((row, rIdx) => (<tr key={rIdx}>{row.map((cell, cIdx) => <td key={cIdx} style={{ padding: '8px', border: `1px solid ${colors.border}` }}>{cell}</td>)}</tr>)) : <tr><td colSpan={sources[0].columns.length} style={{ padding: '8px', textAlign: 'center' }}>(Table empty)</td></tr>}</tbody>
          </table>
        </div>
      </div>
    )}
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ background: colors.sidebar, padding: '15px', borderRadius: '8px', border: `1px solid ${colors.mellowBlue}`, overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
            ...
            <tbody>{results.data.length > 0 ? results.data.map((row, rIdx) => (<tr key={rIdx}>{row.map((cell, cIdx) => <td key={cIdx} style={{ padding: '8px', border: `1px solid ${colors.border}` }}>{cell}</td>)}</tr>)) : <tr><td colSpan={results.columns.length} style={{ padding: '8px', textAlign: 'center' }}>(No data found)</td></tr>}</tbody>
          </table>
        </div>
      </div>
    )}
